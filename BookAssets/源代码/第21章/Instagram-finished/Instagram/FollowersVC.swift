...
class FollowersVC: UITableViewController {
  ...
  func loadFollowers() {
    AVUser.current().getFollowers { (followers:[AnyObject]?, error:NSError?) in
  ...
  func loadFollowings() {
    AVUser.current().getFollowees { (followings:[AnyObject]?, error:NSError?) in
  ...
  override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    ...
    ava?.getDataInBackground({ (data:Data?, error:NSError?) in
      if error == nil {
    ...
    query?.countObjectsInBackground({ (count:Int, error:NSError?) in
      if error == nil {
        if count == 0 {
