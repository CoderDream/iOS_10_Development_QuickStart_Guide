...
class GuestVC: UICollectionViewController, UICollectionViewDelegateFlowLayout {
  ...
  override func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
    ...
    infoQuery?.findObjectsInBackground({ (objects:[AnyObject]?, error:Error?) in
      if error == nil {
        // 判断是否有用户数据
        // guard !objects!.isEmpty else{ print("没有该用户！"); return}
        guard let objects = objects , objects.count > 0 else {
          
          let alert = UIAlertController(title: "\(guestArray.last?.username)", message: "用尽力洪荒之力，也没有发现该用户的存在！", preferredStyle: .alert)
